 ...
 */
const objMap = (obj, map) => Object.entries(obj).reduce((ret, [key,val]) => { ret[key] = map(val, key); return ret; }, {});
 ...
 */
const unique = arr => arr.filter(val => ['boolean','number','string'].includes(typeof(val))).reduce((ret, curr) => { ret.includes(curr.toString()) ? ret : ret.push(curr.toString()); return ret; }, []);
 ...
 * @return {Array}
 */
const reverse = arr => arr.reduceRight((ret, curr) => {ret.push(curr); return ret; }, []);
 ...
 * @return {Array} 
 */
const flatten = arr => arr.map(val => ['boolean','number','string'].includes(typeof(val)) ? val : Object.values(val)).reduce((ret, curr) => { Array.isArray(curr) ? ret.push(...flatten(curr)) : ret.push(curr);  return ret;}, []);
 ...
 * @return {Boolean}
 */
const duckTypeTest = (obj, duck) => Object.entries(duck).reduce((ret, [key, type]) => { ret = ret && obj.hasOwnProperty(key) && typeof(type) === 'object' ? duckTypeTest(obj[key], type) : typeof(obj[key]) === type; return ret; }, true);
